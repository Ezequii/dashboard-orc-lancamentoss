...
export function encodeSharingUrl(url){const bytes=new TextEncoder().encode(url),binary=Array.from(bytes,b=>String.fromCharCode(b)).join('');return'u!'+btoa(binary).replaceAll('/','_').replaceAll('+','-').replace(/=+$/,'')}
