import{SLA}from'../config/designTokens.js';
...
export function exportCsv(records){const columns=['status','diasParado','recebimento','orcamento','fornecedor','solicitante','valorTotal'];const quote=v=>`"${String(v??'').replaceAll('"','""')}"`;return'\ufeff'+[columns.join(';'),...records.map(r=>columns.map(c=>quote(r[c])).join(';'))].join('\n')}
