...
for i,r in enumerate(w.iter_rows(min_row=3,max_col=18,values_only=True),3):
 ...
 x,y,p,e,forn,o,vs,vp,vt,l,osn,req,ped,dp,nf,dnf,s,obs=r;rec=dict(id=i-2,recebimento=c(x),lancamento=c(y),prefixo=c(p),equipamento=c(e),fornecedor=c(forn),orcamento=c(o),valorServico=n(vs),valorPecas=n(vp),valorTotal=n(vt) if c(vt)!=None else n(vs)+n(vp),solicitante=c(l),ordemServico=c(osn),requisicao=c(req),pedido=c(ped),dataPedido=c(dp),nf=c(nf),dataNF=c(dnf),status=st(s),observacoes=c(obs));
 if not rec['fornecedor']: issues.append(f'Linha {i}: fornecedor vazio')
 if rec['valorTotal']<0: issues.append(f'Linha {i}: valor total negativo')
 a.append(rec)
(R/'src/data/orcamentos.json').write_text(json.dumps(a,ensure_ascii=False,separators=(',',':')),encoding='utf-8');now=datetime.now(ZoneInfo('America/Cuiaba'));(R/'src/data/meta.json').write_text(json.dumps({'atualizadoEmTexto':now.strftime('%d/%m/%Y às %H:%M'),'linhasProcessadas':len(a),'inconsistencias':len(issues),'origem':f.name},ensure_ascii=False,indent=2),encoding='utf-8');print(len(a))
